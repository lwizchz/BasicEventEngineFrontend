...
class BEEFBackground(BEEFBaseResource):
	def __init__(self, top, name):
		BEEFBaseResource.__init__(self, top, name)
		self.path = "/resources/backgrounds/"
		self.type = 2
		self.properties = {
			"path": "",
			"width": 0,
	...
	def initPageSpecific(self):
		...
		self.pageAddBitmap("bmp_background", imgpath, (4,0), imgsize=self.getBmpSize((w, h), (128,128)))
	...
	def onButtonSpecific(self, event):
		...
		if bt == self.inputs["bt_ok"]:
			...
			dialog = wx.FileDialog(
				self.top, message="Import Background",
				defaultDir=d,
				defaultFile=f,
				wildcard=wildcards,
				style=wx.FD_OPEN
	...
	def update(self):
		...
		self.inputs["bmp_background"].SetBitmap(wx.BitmapFromImage(img))
	...
	def MenuDuplicate(self, event):
		r = BEEFBackground(self.top, None)
		r.properties = copy.deepcopy(self.properties)
		self.top.addBackground(self.name, r)
